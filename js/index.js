class Index {
    constructor() {
        ...
        // let index = 0;
        //获取到图片的宽度
        this.imgW = this.$('.nav-lunbo').offsetWidth;
        // console.log(this.imgW);
        //获取到所有的.nav-lunbo下面的所有Li标签
        this.lunboLisObj = document.querySelectorAll('.nav-lunbo li');
        // console.log(this.lunboLisObj[0]);
        //一进来设置下方按钮
        this.button()
        //获取到左右按键并绑定点击事件 
        this.$('.arrow-left').addEventListener('click', this.arLfn.bind(this));
        // console.log(this.$('.arrow-left'));
        this.$('.arrow-right').addEventListener('click', this.arRfn.bind(this));
        //用于清除定时器
        this.time;
        this.time1;
        //自动切换
        this.zidongFn()
        //设置鼠标移入navlunbo时停止定时器
        this.$('.nav-lunbo').addEventListener('mouseenter', this.closeFn.bind(this))
        this.$('.nav-lunbo').addEventListener('mouseleave', this.openFn.bind(this))
        //图片的切换效果
        this.qihuan()
        //设置移入切换热门和更多
    ...
    tiaoFn() {
        location.href = './views/shoping.html'
    }
    deFn() {
    ...
    button() {
        ...
        let ol = this.$('ol')
        // console.log(this.$('ol'));
        for (let i = 0; i < this.lunboLisObj.length; i++) {
            let index = 0;
            //生成按钮
            let olLi = document.createElement('li');
            olLi.setAttribute('id', i);
            // olLi.innerHTML = i + 1;
            ol.appendChild(olLi)
            //此时判断是否下标为0 用于设置class current
            if (i == 0) {
                olLi.classList.add('current');
            }
            //并且设置点击事件
            olLi.onclick = that.olFn.bind(this)
        }
        //克隆一张图片并且让她直接跳过去
        let clone = this.lunboLisObj[0].cloneNode(true);
        this.$('.nav-lunbo ul').appendChild(clone);
        // console.log(clone);
    }
    //设置下标
    xiabiao(index) {
        //一进来就要清除所有的ClassName current
    ...
    olFn() {
        ...
        let target = (this.imgW * index) + 38;
        // index = index + i;
        // // console.log(index);
        // //设置图片的位置的变化 1个图片为1226
        // // console.log(that.imgW);
        // // console.log(target);
        this.move(this.$('.nav-lunbo ul'), {
            left: -target
        })
        this.xiabiao(index)
    }
    ...
    arLfn() {
        clearInterval(this.time);
        // console.log(this.$('.current').getAttribute('id'));;
        let index = this.$('.current').getAttribute('id');
        // 遍历li
        index--;
        console.log(index);
        //直接显示最后一章
        if (index == -1) {
            this.$('.nav-lunbo ul').style.left = -(document.querySelectorAll('ol li').length - 1) * this.imgW + 'px';
            // 设置下标
            index = document.querySelectorAll('ol li').length - 1;
            // console.log(index);
        }
        //设置位移
        let target = this.imgW * index + 38;
    ...
    arRfn() {
        ...
        //获取移动距离
        let target = '';
        //设置一个开关用于判断是不是道道倒数第二张图片 用于进行设置位移0
        let s = false
        //判断是否到达最后那一图片
        if (index == this.lunboLisObj.length - 1) {
            // this.lunboLisObj = document.querySelectorAll('.nav-lunbo li');
            index++;
            //此时在获取一次li
            // console.log(this.lunboLisObj);

            target = this.imgW * index + 38;
        ...
        this.move(this.$('.nav-lunbo ul'), {
            ...
            if (s) {
                that.$('.nav-lunbo ul').style.left = 0;
            }
        })
        this.xiabiao(index)
    }
