class Index {
    constructor() {
        ...
        //获取到option-box的ul 并绑定点击事件
        this.ulObjs = document.querySelectorAll('.option-box ul');
        //选择套装也是同理
        this.$('.batch-box ul').addEventListener('click', this.ulFn.bind(this))
        this.ulObjs.forEach(v => {
            v.addEventListener('click', this.ulFn.bind(this))
        })

        //加入购物车发送loalStrage数据
        this.$('.btn2').addEventListener('click', this.sendFn.bind(this));
        // console.log(this.$('.service-box'));
        this.count = 0;
        //小米意外保险以及其他服务点击事件
        this.$('.service-box').addEventListener('click', this.serFn.bind(this))
        //用于判断有没有这个节点没有就不执行 否则就执行点击事件
        if (!this.$('.delte')) {
            return
        } else {
    ...
    serFn() {
        ...
        // console.log(event.target.parentNode.querySelectorAll('li'));
        event.target.parentNode.querySelectorAll('li').forEach(v => {
            v.className = '';
        })
        //点击哪个就给谁添加active2
    ...
    //发送localStorage数据
    sendFn() {
        //选中.activ2包含的price 进行传递给LocalStorage
        let a = 0;
        let tPrice = document.querySelectorAll('.active2 .price');
        // console.log(tPrice);
        tPrice.forEach(v => {
            a += Number((v.innerHTML).replaceAll(/[\u4e00-\u9fa5]/g, ""));
        })
        // console.log(a);
        let price = this.$('.total-price').innerHTML;
        //去除掉汉字和特殊字符
        price = price.replaceAll(/[\u4e00-\u9fa5]/g, "");
        price = Number(price.replace(/\W+/gi, ''));
        if (a) {
            price += a;
        }
        // console.log(price);
        let num = 1;
        let name = this.$('.product-con h2').innerHTML;
        // console.log(this.$('.img-box img').src);
        let src = this.$('.img-box img').src;
        let cart = {
            id: 21,
            name,
            price,
    ...
    ulFn() {
        // console.log(event.target.parentNode.parentNode.querySelectorAll('li'));
        //清除所有的li的class
        let lis = event.target.parentNode.parentNode.querySelectorAll('li');
        lis.forEach(v => {
            v.className = '';
        })
        event.target.parentNode.className = 'active3'
    }
    tiaoFn() {
    ...
    addActive(index) {
        ...
        this.inputObj.placeholder = this.lisObj[index].innerText;
    }
    //给下拉框绑定一个键盘按下事件
    downFn() {
